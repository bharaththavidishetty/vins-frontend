...
const Secondpage = () => {
  ...
  const handleSendMobileOTP = async () => {
    if (/^[6-9]\d{9}$/.test(formData.mobile)) {
      try {
        const response = await axios.get(`http://192.168.1.47:9090/customer/get/${formData.mobile}`);
  ...
  //     try {
  //       const response = await axios.post(`http://192.168.1.47:9090/vehicle/sendEmailOTPforUpdation/${formData.email}`);
  ...
  const handleSendEmailOTP = async () => {
    if (/^[a-z0-9._%+-]+@gmail\.com$/.test(formData.email)) {
      try {
        const response = await axios.get(`http://192.168.1.47:9090/customer/getByEmail/${formData.email}`);
        console.log(response);
        if (response.data) {
      ...
      setEmailOtpError('Please enter a valid email number');
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(`http://192.168.1.47:9090/customer/add`, formData, {
        headers: {
            ...
            <form className='sp-form' onSubmit={handleSubmit}>
              ...
              <div className="sp-form-group ">
                ...
                <input
                  ...
                  autoComplete='off'
                />
                <button type="button" className="sp-otp-button" onClick={handleSendMobileOTP} disabled={isEmailOTPSent}>Send OTP</button>
              ...
              <div className="sp-form-group">
                ...
                <input
                  ...
                  autoComplete='off'
                />
                <button type="button" className="sp-otp-button" onClick={handleSendEmailOTP} disabled={isEmailOTPSent}>Send OTP</button>
...
//     if (/^[a-z0-9._%+-]+@gmail\.com$/.test(formData.email)) {
//       try {
//         const response = await axios.post(`http://192.168.1.95:9090/vehicle/sendEmailOTPforUpdation/${formData.email}`);
...
//     try {
//       const response = await axios.post(`http://192.168.1.95:9090/customer/add`, formData, {
//         headers: {
//           'Content-Type': 'application/json',
//         },
//       });
