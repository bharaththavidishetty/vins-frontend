...
function PaymentPage() {
  ...
  useEffect(() => {
    const fetchCustomerData = async () => {
      try {
        const response = await axios.get(`http://192.168.1.95:9090/customer/get/${formData.mobile}`);
  ...
  const sendPaymentDetails = async () => {
    try {
      const response = await axios.post(`http://192.168.1.95:9090/payment/add`, customerData);
      if (response.status === 200) {
